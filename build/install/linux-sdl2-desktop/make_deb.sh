#!/bin/bash
#
# Build a source-based .deb for Unreal Speccy Portable (SDL2 + desktop OpenGL + Dear ImGui variant)
#
# debian/ stays in build/install/linux-sdl2-desktop/debian/
# Creates a temporary symlink from the repository root (removed on exit).
#
# Parallel to build/install/linux-wx/make_deb.sh — both packages coexist under
# different names (unreal-speccy-portable, unreal-speccy-portable-wx and
# unreal-speccy-portable-sdl2-desktop) and can be built/installed side by side.
#
#   ./build/install/linux-sdl2-desktop/make_deb.sh
#
# CI: collect packages into a directory
#   OUTPUT_DIR="${GITHUB_WORKSPACE}/artifacts" ./build/install/linux-sdl2-desktop/make_deb.sh
#
# Version from git:
...
{
    echo "unreal-speccy-portable-sdl2-desktop (${DEB_VERSION}) unstable; urgency=medium"
    echo
...
echo "==> Building source-based .deb (SDL2 desktop variant) ..."
...
while IFS= read -r f; do
    DEBS+=("$f")
done < <(find "$PARENT" -maxdepth 1 -name "unreal-speccy-portable-sdl2-desktop_${DEB_VERSION}_*.deb" 2>/dev/null | sort)

if [ ${#DEBS[@]} -eq 0 ]; then
while IFS= read -r f; do
    DEBS+=("$f")
done < <(find "$PARENT" -maxdepth 1 -name 'unreal-speccy-portable-sdl2-desktop_*.deb' 2>/dev/null | sort)
fi

if [ ${#DEBS[@]} -eq 0 ]; then
    while IFS= read -r f; do
        DEBS+=("$f")
    done < <(find "$REPO_ROOT" -maxdepth 1 -name 'unreal-speccy-portable-sdl2-desktop_*.deb' 2>/dev/null | sort)
fi
